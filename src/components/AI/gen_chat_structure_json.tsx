...
const FormatMessageContent = ({ content }) => {
    try {
        ...
        // If "DocDialogOutput" exists and is also a JSON string, parse it
        if (parsedContent?.DocDialogOutput) {
            try {
                parsedContent.DocDialogOutput = JSON.parse(parsedContent.DocDialogOutput);
            } catch (e) {
                console.warn("Failed to parse nested JSON inside DocDialogOutput", e);
            }
        }

        return (
            <pre className="bg-gray-900 text-green-400 p-3 rounded-md text-sm overflow-auto">
                <code>{JSON.stringify(parsedContent, null, 2)}</code>
            </pre>
        );
    } catch (e) {
        // If content is not JSON, fall back to normal text & link formatting
        console.warn("Content is not valid JSON, falling back to default formatting", e);
