...
export default function ClientPage() {
  ...
  const [activeFilter, setActiveFilter] = useState('MPA'); // Default filter value
  const ActiveComponent = getActiveComponent(activeComponent);

  useEffect(() => {
    // Restore filter value from localStorage
    const savedFilter = localStorage.getItem('filter') || 'MPA';
  ...
  const filteredItems = NAV_ITEMS.mainComponents.filter((item) =>
    activeFilter === 'MPA' ? item.visible_mpa : item.visible_arzt
  );
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-center mb-4">
            <button
              ...
              className={`px-4 py-2 rounded-l ${activeFilter === 'MPA' ? 'bg-gray-500 text-white' : 'bg-gray-400 text-black'
                }`}
            >
              MPA
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-r ${activeFilter === 'Arzt' ? 'bg-gray-500 text-white' : 'bg-gray-400 text-black'
                }`}
            >
              Arzt
            </button>
                ...
                <MenuItems className="absolute z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                  <div className="py-1">
                    {NAV_ITEMS.summariesDropdown
                      .filter((item) => (activeFilter === 'MPA' ? item.visible_mpa : item.visible_arzt))
                      .map(({ key, name }) => (
                        <MenuItem key={key}>
                          {({ active }) => (
                            <a
