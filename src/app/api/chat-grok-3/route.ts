...
// moder Xai grok 2, soon 3, adjust  below
...
const openai = new OpenAI({
  ...
  baseURL: 'https://api.x.ai/v1', // Updated base URL
});
  ...
  try {
    ...
    if (!process.env.XAI_API_KEY) {
      ...
      return new NextResponse(
        JSON.stringify({ error: 'Missing xAI API Key.' }),
        { status: 400 }
      );
    }
    ...
    const model = 'grok-2-latest'; // adjust to 3 once grok 3 is api available
